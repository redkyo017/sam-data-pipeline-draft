...
class S3WriteError extends Error {
  constructor(m, d) {
    super(m);
    this.name = "S3WriteError";
    this.details = d;
  }
}
...
const deleteSources = async (sources, outBucket, outKey) => {
  ...
  console.log(
    "Files to delete:",
    sources.map((s) => `s3://${s.bucket}/${s.key}`),
  );
  ...
  console.log(
    `Cleanup completed: attempted=${attempted}, deleted=${deleted}, errors=${errors.length}`,
  );
  ...
  try {
    ...
    if (!sources || sources.length === 0) {
      throw new InputValidationError(
        "Provide sources as [{bucket,key},...] or {bucket, keys: [...]}",
        { eventSnippet: Object.keys(event || {}) },
      );
    }

    const outBucket = process.env.OUTPUT_BUCKET;
    if (!outBucket)
      throw new InputValidationError("Missing OUTPUT_BUCKET env var.");

    // Use the new path structure: campaigns/{campaign_id}/{commit_id}/{execution_id}.json
    const outKey =
      event.outputKey ||
      `campaigns/${campaign_id}/${commit_id}/${executionId}.json`;
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          if (!firstFileProcessed && data.length > 0) {
            campaign_id =
              campaign_id || data[0].campaign_id || "unknown-campaign";
            commit_id = commit_id || data[0].commit_id || "unknown-commit";
            firstFileProcessed = true;
            console.log(
              `Using campaign_id=${campaign_id}, commit_id=${commit_id}`,
            );
          }

          // Transform each record to OUTPUT_DATA_STRUCTURE format
          const transformedData = data.map((record) =>
            transformToOutputStructure(record, campaign_id, commit_id),
          );
          allData.push(...transformedData);
        } else {
          console.warn(
            `File s3://${bucket}/${key} does not contain a JSON array, skipping`,
          );
        }
      } catch (parseError) {
        throw new S3ReadError(
          `Failed to parse JSON from s3://${bucket}/${key}`,
          {
            bucket,
            key,
